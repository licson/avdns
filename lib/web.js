...
WebServer.prototype.prepareAdminPage = function () {
	...
	this.app.post('/admin/settings', loggedIn, function (req, res) {
		Object.keys(req.body).forEach(function (opt) {
			if (self.config.config.has(opt)) {
				self.config.update(opt, req.body[opt]);
			}
		});
	...
	var updateMemory = function (str) {
		sseClients.forEach(function (res) {
			res.write(`event: mem\ndata: ${process.memoryUsage().heapUsed / 1024 / 1024}\n\n`);
		});
	}
	...
	var sseClients = [];
	this.app.get('/admin/channels', loggedIn, function (req, res) {
		sseClients.push(res);
