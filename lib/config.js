...
var Config = function () {
	...
	db.hgetallAsync('config').then(function (opts) {
		if ((opts !== null || typeof opts === "undefined") && Object.keys(opts).length > 0) {
			Object.keys(opts).forEach(function (opt) {
				self.emit('config:' + opts, JSON.parse(opts[opt]));
				self.config.set(opt.replace('config:', ''), opts[opt]);
			});
		}
	});
	...
	this.conn.on("pmessage", function (pat, channel, message) {
		try {
			...
			self.config.set(channel.replace('config:', ''), JSON.parse(message));
		} catch (e) {
			console.error(e);
		}
	});
...
Config.prototype.update = function (config, value) {
	...
	conn
		.publishAsync('config:' + config, value)
		.then(function () {
			conn.quit();
		});
	
	db.hset('config', config, value);
