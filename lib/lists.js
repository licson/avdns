...
var ListChecker = function (cChannel) {
	var self = this;
	...
	this.dnsbl = [
		...
		// Too much DNSBL checks slows down responses
	...
	cChannel.subscribe([
		{ name: 'dnsbl_list', value: Object.assign([], this.dnsbl) },
		{ name: 'dnsbl_ratio', value: 0.3 },
		{ name: 'whitelist', value: [] }
	]);

	cChannel.on("config:dnsbl_list", function (list) {
		self.dnsbl = list;
	});

	cChannel.on("config:dnsbl_ratio", function (ratio) {
		self.ratio = ratio;
	});

	cChannel.on("config:whitelist", function (list) {
		self.whitelist = list;
	});
}
...
ListChecker.prototype.checkName = function (name) {
	...
	return redis.getAsync("blacklist:" + name).then(function (res) {
		if (res === null || !!res === false) {
			return Promise.resolve(false);
		} else if (self.whitelist.indexOf(name) > -1) {
			return Promise.resolve(false);
		} else {
			return Promise.resolve(true);
		}
